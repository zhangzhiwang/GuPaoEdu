...
@FeignClient(name = "GuPaoEdu-SpringCloudNetflix-Provider-Product", 
			configuration = LogConfig.class// // 声明该类使用Feign客户端，参数是服务生产者的应用名称。Feign内置了Ribbon负载均衡器，通过服务生产者的应用名称从服务注册中心获取到生产者列表再通过内置的负载均衡算法得到被调用的ip和端口
					// Feign比使用RestTemplate更友好，更符合面向对象的特性
//			fallback = FeignHystrixFallbackService.class
		)
